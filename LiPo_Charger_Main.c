 ...
 * Version: 1.5
 *
 * Description: Texas Instruments MSP430FR6989 Development Board used and a 1300mAh 6-cell Lithium Polymer battery used.
 *              Program prompts user to either bypass or enable input fault conditions for the temp limit (40C-45C) and
 *              timer limit (5-60 minutes) in intervals 5. Safety Features (Continuously Checks for over-current at 3A
 *              and over-under-voltage and faulty cells). Program auto detects the number of cells in the battery
 ...
 *              battery pack (0.1 * 1300mA == 130mA), charging has completed.
 *
 *
 * Failure Mode Effects Analysis: Hazards: Over and under voltage/current.
 *                             Mitigation: Redundancy in the form  loops to detect over/under current
...
int DecVal = 0, TempLimit = 40, TimeLimit = 5, TimeLimit2 = 0, startup,
        halt = 0, count = 0, SetConditions = 0, k, loop1 = 1, testval2;
...
void setup()
{
    ...
    /******************************************************************************
     * Initialization of rotary encoder & associated function decleration
     ******************************************************************************/
    ...
    /******************************************************************************
     * Initialization of timerA & timerB
     ******************************************************************************/
    ...
    /******************************************************************************
     * Initialization of onboard LEDs & buttons
     ******************************************************************************/
    //Red TimerB
    P1DIR |= BIT0;                          // for LED1
    ...
    P1REN |= BIT2;                       // Enable pull up resistor for button 2
    P1DIR |= BIT2;
    P1OUT |= BIT2;

    P1REN |= BIT1;                       // Enable pull up resistor for button 1
    P1DIR |= BIT1;
    P1OUT |= BIT1;

    /******************************************************************************
     * Initialization of CC & CV Signal Pins (Solid State Relays)
     ******************************************************************************/
...
void loop()
{
    ...
    CellAutoDetect();           // Auto-detect the number of cells connected
    ...
    else
    {
        ...
        lcd.clear();
        while (digitalRead(RotSw) == 1) // Default condition for Temp & Time limit
        {
            TempLimit = 45;             // default temp limit is 45 Celsius
            TimeLimit = 3600;           // default time limit is 60 minutes
    ...
    while (1)
    {
        ...
        while (BattCurrent < 3)
        {
            // if battery temp is less than or equal to temp limit
            if (AvgTemp <= TempLimit)
            {

                // if the timer is less than the user defined or default time limit
                if (Timer < TimeLimit)
                {
                    ...
                    if (!BattFullCharge)
                    {
                        ...
                        /*Loop to display number of cells connected, cell voltages,
                         and charge current for 5 seconds*/
                        if (testVal < 5)
                        {
                            // Calculates the true voltage of each cell
                            TrueCellVoltage(ADC12MEM1, ADC12MEM2, ADC12MEM3,
                                            ADC12MEM4, ADC12MEM5, ADC12MEM6);

                            // Determines the amount of cells connected
                            CellAutoDetect();

                            if (faultFlag3) // Condition check for cell over voltage 4.25V
                            {
                                lcd.setCursor(0, 0);
                            ...
                            else // if cells are below 4.25V
                            {

                                switch (NumberOfCells)
                                // Based on the respective number of cells attached
                                {

                                case 0:                   // Zero cells detected
                                ...
                                default:             //For Failure Mode Analysis
                                    lcd.setCursor(19, 3);
                                    lcd.print("x");
                                }
                            }
                        ...
                        // enter if number of cells is not zero
                        if (NumberOfCells != 0)
                        {
                            // Loop for six seconds
                            if ((testVal >= 6) && (testVal < 12))
                            {
                                // Prints current value of the timer
                                lcd.setCursor(5, 1);
                                lcd.print("Time:");
                                lcd.setCursor(11, 1);
                                lcd.print(hr);
                                ...
                                // Prints the battery pack temperature
                                lcd.setCursor(0, 2);
                                lcd.print("Pack Temp:");
                                lcd.setCursor(16, 2);
                                lcd.print(char(0xDF));  //degree symbol
                                ...
                                // Remeasure the cell voltages
                                TrueCellVoltage(ADC12MEM1, ADC12MEM2, ADC12MEM3,
                                                ADC12MEM4, ADC12MEM5,
                                                ADC12MEM6);

                                // Calculate the average cell pack voltage
                                AvgPackCharge = (Cell0TV + Cell1TV + Cell2TV
                                        + Cell3TV + Cell4TV + Cell5TV) / 6;

                                // Prints the average pack voltage
                                lcd.setCursor(0, 3);
                                lcd.print("Average V:");
                                lcd.setCursor(11, 3);
                        ...
                        if (!BattFullCharge) // Enter main charging algorithm if battery
                        {
                            ...
                            // Function to trace which cell has which voltage
                            CellVoltageTrace(CellVoltageArray, 6, CellArr);
                            AvgPackCharge = (ADC12MEM1 + ADC12MEM2 + ADC12MEM3
                                    + ADC12MEM4 + ADC12MEM5 + ADC12MEM6) / 6;

                            // If any of the cells reach 2.0V == 0x9B2 (raw input 4.0V), Start Constant Voltage
                            //3.90V                     //4.0V
                            if ((AvgPackCharge > 0x973)
                                    && (AvgPackCharge < 0x9B2)) // For Failure Mode Analysis if average cell voltage was between 3.90V - 4.00V
                            {
                                P2OUT &= ~BIT4; //disable Constant Current

                                // enable the constant voltage Solid State Relay P2.5
                                P2OUT |= BIT5;
                                P9OUT |= BIT7; // green led flash when on
                                P1OUT &= ~BIT0;
                            }

                            if (AvgPackCharge < 0x973) // If the average cell voltage is 3.90v
                            {
                                P2OUT &= ~BIT5; // disable the constant voltage Solid State Relay P2.5
                                P2OUT |= BIT4; // enable Constant Current Solid State Relay P2.4
                                P1OUT |= BIT0;
                            ...
                            if (AvgPackCharge > 0x9B2) // Once average battery pack was 4.0V, deemed fully charged
                            {
                    ...
                    // while battery is fully charged
                ...
                // fault condition for when time limit has been reached
                else
                {
                    //Print to screen the TimeLimit Reached
                    lcd.setCursor(1, 2);
            ...
            // fault condition for when the temp limit has been reached
            else
            {
        ...
        // If battery current exceeds the 3A
...
/******************************************************************************
 * Calculates the actual temperature from the thermocouples and produces the
...
/******************************************************************************
 * Calculates the true cell voltage by multiplying the reciprocal of the ADC
 * step size by the ratio of the voltage divider network attached to the
...
/******************************************************************************
 * Calculates the charge current to the entire battery pack front the low side
 * of the battery pack with a current sense resistor @ .005 Ohms.
 ******************************************************************************/
void CellCurrent(int a)
{
    A = 1240.69; // reciprocal of 12-Bit ADC step size (.000806)^-1 = 1240.69
 ...
 * Primary Purpose: Increment the testVal variable which is the timer increment
 * once every second.
 * Redundant software checks fault condition: templimit,
 *
 ******************************************************************************/
